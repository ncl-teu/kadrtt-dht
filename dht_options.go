...
type config struct {
	...
	kadrtt_cache_interval time.Duration
	kadrtt_lambda_leave   float64
	kadrtt_lambda_lookup  float64
...
var defaults = func(o *config) error {
	...
	o.kadrtt_cache_interval = time.Duration(100) * time.Second
	o.kadrtt_lambda_leave = 0.1
	o.kadrtt_lambda_lookup = 0.1
...
func SetKadRTTCacheInterval(value int) Option {
	return func(c *config) error {

		c.kadrtt_cache_interval = time.Duration(value) * time.Second

		return nil
	}
}

func SetKadRTTLambdaLeave(value float64) Option {
	return func(c *config) error {
		c.kadrtt_lambda_leave = value
		return nil
	}
}

func SetKadRTTLambdaLookup(value float64) Option {
	return func(c *config) error {
		c.kadrtt_lambda_lookup = value
		return nil
	}
}
